...
const Nav = ({ Search, setSearch, setcart, cartItems }) => {
  ...
  return (
    <div className="w-full h-15 md:h-25 bg-red-200 flex justify-between items-center px-2 md:px-8">
      ...
      <form className="w-[70%] rounded-md shadow-md h-[60%] bg-white flex items-center px-5 gap-5">
        ...
        <input
          className="w-full md:text-[20px] text-[15px] outline-none"
          type="text"
      ...
      <div className="w-10 md:w-15 relative h-10 md:h-15 rounded-md shadow-xl bg-white flex justify-center items-center">
        {cartCount > 0 && (
          <span className="absolute top-0 right-1 md:top-2 text-white bg-red-500 font-bold md:text-[18px] text-[12px] w-5 h-5 flex items-center justify-center rounded-full">
            {cartCount}
          </span>
        )}
        <LuShoppingBag
          onClick={() => setcart(true)}
          className="md:w-7.5 md:h-7.5 top-3 h-4 w-4 text-green-500"
        />
      </div>
    </div>
  );
